...
class daq:
        ...
        for address in range(128):
            try:
                ...
                if address == 0x6a or address == 0x6b:
                    ...
                    self.devices_config[address] = [0x00, 2, '>h', ['cur'], None]
                    ...
                    self.devices_config[address] = [0x0C, 2, '>H',['rot'], None]
    ...
    def calibrate_imu(acc=None, gyr=None, Ts=None, Td=None, fs=None):
        ...
        if acc is not None:        
            acc_mean = np.array([np.mean(acc[Ns*n:Ns*(n+1)-1], axis=0) for n in range(6)]).T
        ...
        if gyr is not None:
            ...
            gyr_std = np.std(gyr[:6*Ns,:],axis=0).reshape((3,1))
